...
class ClientesModel extends Model{
    ...
    protected $allowedFields = ['id_tercero','email_facturacion','nombre_contacto', 'rep_legal','email_replegal','activo','usuario_crea','sucursal']; /* relacion de campos de la tabla */
    ...
    public function traerCliente($id) {
        $this->select('clientes.*,t.tipo_documento,t.numero_documento,t.razon_social,t.direccion,t.tipo_tercero,d.resumen,v.valor as v_resumen,dp.nombre as departamento,m.nombre as municipio');
        $this->join('terceros as t','id_tercero=t.id');
        $this->join('detalleparametros as d','d.id=t.tipo_documento');
        $this->join('v_detalleparametros as v','v.id=t.tipo_tercero');   
        $this->join('departamentos as dp','dp.id=departamento');
        $this->join('municipios as m','m.id=municipio'); 
        $this->where('clientes.id',$id);
        $datos = $this->first();
        return $datos;
    }    
}
